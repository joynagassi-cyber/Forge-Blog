/**
 * Edge Function: ai-draft-generation (section 9.2 / 14.4)
 *
 * POST body: { article_id: string }
 *
 * Requires article.brief to be populated (run ai-brief-generation first).
 * Produces a full section-10 scaffold JSON into articles.content (draft only).
 * Status is advanced to "drafting". Human must approve before "in_review".
 */
...
async function callProvider(provider: any, systemPrompt: string, userPrompt: string, maxTokens = 4096) {
...
Deno.serve(async (req) => {
  ...
  const { data: article, error: articleErr } = await supabase
    .from("articles")
    .select("id, working_title, locale, brief, pillar_id, pillars(name_en, name_fr, slug, target_product)")
    .eq("id", article_id)
    .single();
  ...
  if (!article.brief || Object.keys(article.brief).length === 0) {
    return json({ error: "Brief is missing. Run ai-brief-generation first." }, 422);
  }
  ...
  const { data: providers } = await supabase
    ...
    .select("*")
    .eq("is_active", true)
    .contains("assigned_tasks", ["draft_generation"]);

  const provider = providers?.[0];
  if (!provider) {
    return json({ error: "No active AI provider assigned to draft_generation." }, 503);
  }
  ...
  try {
    draftText = await callProvider(provider, systemPrompt, userPrompt, 6000);
  } catch (err: any) {
  ...
  try {
    ...
    return json({ error: "Model returned invalid JSON. Try again or check the provider.", raw: draftText.slice(0, 500) }, 422);
  }
  ...
  if (updateErr) {
    return json({ error: "Failed to save draft content", detail: updateErr.message }, 500);
  }
